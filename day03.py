...
print("지금 저금통에있는 {}원이 있습니다.".format(piggy_bank))
...
문제4-3 : 네자리 정수로 구성된 사원번호를 기준으로 근무시간을 결정하고자 한다.
...
문제4-5 : 국어 영어 수학 점수를 각각 입력받아 평균을 구하고
평균이 80점 이상이면 합격 아니면 불합격을 출력한다.

국어 점수를 입력하세요>>
영어 점수를 입력하세요>>
수학 점수를 입력하세요>>
평균은 83.0 이고 결과는 합격입니다.

kor = int(input("국어 점수를 입력하세요>>"))
eng = int(input("영어 점수를 입력하세요>>"))
mat = int(input("수학 점수를 입력하세요>>"))
avg = (kor+eng+mat)/3 #평균을 구하는 연산을 처리(산술 연산자)
result = '합격' if avg>=80 else '불합격'
print("평균은 {0} 이고 결과는 {1}입니다".format(avg,result))

@if else 문 심화 : input 을 이용한 if문을 만들자
몇살입니까?>>20
성인
몇살입니가?>>19
미성년자
...
@if elif else 문 : input을 이용한 if - alif문을 만들자
몇살입니까?>>5
미취학
몇살입니까?>>10
초등학생
...
1. 비밀번호를 만든다.
2. 비밀번호를 입력한다.
조건물에 비밀번호를 추가해서 비밀번호가 맞다면 관리자이다.

id = input("아이디를 입력하세요:>>")
level = int(input("회원레벨을 입력하세요:>>"))
pw = int(input("비밀번호를 입력하세요.>>"))
if id =='admin' and level ==1 and pw ==1234:
    print("관리자입니다.")
else:
    print("일반 회원입니다.")


문제 : 조건문으로 계절를 판단하는 프로그램을 만들어보자
월을 입력하세요:>>
1. 입력하는 월이 3과 같거나 크면서 5와 같거나 작으면 봄이다.
2. 입력하는 월이 6과 같거나 크면서 8과 같거나 작으면 여름이다.
3. 입력하는 월이 9보다 같거나 크면서 11보다 같거나 작으면 가을이다.
4. 입력하는 월이 12보다 같거나 크면서 2보다 같거나 작으면 겨울이다.
...
좌석의 종류를 입력하세요(1: 일반실, 2:특실):>>1
일반실입니다.
좌석의 종류를 입력하세요(1: 일반실, 2:특실):>>2
특실입니다.

"""
sit = int(input("좌석의 종류를 입력하세요:>>"))
if sit==1:
    print("일반실 입니다.")
if sit==2:
    print("특실 입니다.")
